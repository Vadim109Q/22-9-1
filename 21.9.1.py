...
# Проверка условий ввода данных.

if " " not in sequence_numbers:
    print("\nВ вводе отсутствуют пробелы (введите числа, согласно условию.)")
    sequence_numbers = input('Введите любые целые числа через пробел: ')
if not is_int(sequence_numbers):
    print('\nВ вводе содержаться не числа (введите числа, согласно условию.)\n')
    print(error)
else:
...
# Определение позиции введенного элемента
...
def binary_search(array, element, left, right):
    try:
        ...
        return 'Число превышает диапозон списка.'

# Определяется номер позиции элемента, который меньше введенного числа, а следующий за ним больше или равен этому числу
